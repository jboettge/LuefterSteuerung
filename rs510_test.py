...
# Detect the slave-address keyword for the installed pymodbus version:
#   <3.0 → unit=   3.0–3.12 → slave=   3.13+ → device_id=
...
async def read_status(client: AsyncModbusSerialClient, slave: int) -> None:
    ...
    try:
        result = await client.read_holding_registers(
            REG_MONITOR_START, count=REG_MONITOR_COUNT, **_sk(slave),
        )
...
async def read_param_status(client: AsyncModbusSerialClient, slave: int) -> None:
    ...
    for addr, desc, div, unit in params:
        try:
            result = await client.read_holding_registers(addr, count=1, **_sk(slave))
            if result.isError():
...
async def write_cmd(client: AsyncModbusSerialClient, slave: int, cmd: int, label: str) -> None:
    try:
        result = await client.write_register(REG_CONTROL_CMD, cmd, **_sk(slave))
        if result.isError():
...
async def set_frequency(client: AsyncModbusSerialClient, slave: int, freq_hz: float) -> None:
    ...
    try:
        result = await client.write_register(REG_FREQ_SETPOINT, value, **_sk(slave))
        if result.isError():
            print(f"Schreibfehler Frequenz (0x{REG_FREQ_SETPOINT:04X}): {result}")
            return
        print(f"OK: Frequenz {freq_hz:.2f} Hz gesetzt (0x2502 = {value})")
        result2 = await client.write_register(REG_CONTROL_CMD, CMD_RUN_FORWARD, **_sk(slave))
        if result2.isError():
...
async def read_input_registers(client: AsyncModbusSerialClient, slave: int, start_hex: str, count: int) -> None:
    ...
    try:
        result = await client.read_input_registers(addr, count=count, **_sk(slave))
        if result.isError():
...
async def write_coil(client: AsyncModbusSerialClient, slave: int, addr_hex: str, on: bool) -> None:
    ...
    try:
        result = await client.write_coil(addr, on, **_sk(slave))
        if result.isError():
...
async def run_heartbeat(
    ...
    try:
        iteration = 0
        while True:
            r1 = await client.write_register(REG_FREQ_SETPOINT, freq_value, **_sk(slave))
            r2 = await client.write_register(REG_CONTROL_CMD, CMD_RUN_FORWARD, **_sk(slave))
        ...
        print("\nStoppe Motor ...")
        await client.write_register(REG_CONTROL_CMD, CMD_STOP, **_sk(slave))
...
async def write_param(client: AsyncModbusSerialClient, slave: int, addr_hex: str, value: int) -> None:
    addr = int(addr_hex, 16)
    try:
        result = await client.write_register(addr, value, **_sk(slave))
        if result.isError():
...
async def write_param_fc16(client: AsyncModbusSerialClient, slave: int, addr_hex: str, value: int) -> None:
    ...
    addr = int(addr_hex, 16)
    try:
        result = await client.write_registers(addr, [value], **_sk(slave))
        if result.isError():
...
async def main() -> None:
    ...
    try:
        ...
        if action == "status":
            ...
            try:
                result = await client.read_holding_registers(
                    start_addr, count=count, **_sk(args.slave),
                )
                if result.isError():
